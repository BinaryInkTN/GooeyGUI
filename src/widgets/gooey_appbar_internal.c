...
void GooeyAppbar_Internal_Draw(GooeyWindow* window)
{
    ...
    active_backend->DrawGooeyText(10, APPBAR_HEIGHT/2 + active_backend->GetTextHeight(window->appbar->appname, strlen(window->appbar->appname)) / 2, window->appbar->appname, window->active_theme->neutral, 0.4f, window->creation_id, NULL);
}
