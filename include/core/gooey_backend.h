...
typedef struct GooeyBackend
{
    ...
    void (*UpdateBackground)(GooeyWindow* win);
    void (*Clear)(GooeyWindow *win);
    ...
    unsigned int (*LoadImage)(const char* image_path);
    void (*DrawImage)(unsigned int texture_id, int x, int y, int width, int height, int window_id);
